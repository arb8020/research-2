...
Credit for the idea goes to the [SWE book](https://abseil.io/resources/swe-book). I always felt that integration versus unit debate is confused, the book helped me to formulate in which way exactly.
...
_The second_ axis is the fraction of the code which gets exercised, potentially indirectly, by the test. Does the test exercise only the business logic module, or is the database API and the HTTP handling also required? This is _distinct_ from performance: running more code doesn't mean that the code will run slower. An infinite loop takes very little code. What affects performance is not whether tests for business logic touch persistence, but whether, in tests, persistence is backed by an in-memory hash-map or by an out-of-process database server.
...
If you enjoyed this post, you might like [_How to Test_](https://matklad.github.io/2021/05/31/how-to-test.html) as well. It goes further in the prescriptive direction, but, when writing it, I didn't have the two dimensional purity-extent vocabulary yet.
...
Also, as a pitch for the _How to Test_ post, take a second to appreciate how simple the test is, considering that it tests an error-resilient, highly incremental compiler :)
