---
title: "How to Test"
site: "matklad.github.io"
published: 2021-05-31
source: "https://matklad.github.io/2021/05/31/how-to-test.html"
domain: "matklad.github.io"
language: "en-US"
description: "Alternative titles:  Unit Tests are a Scam  Test Features, Not Code  Data Driven Integrated Tests"
word_count: 4295
---
...
Good code [is easy to delete](https://programmingisterrible.com/post/139222674273/how-to-write-disposable-code-in-large-systems). Tests represent an investment into existing code, and make it costlier to delete (or change).
...
There’s one talk about software engineering, which stands out for me, and which is my favorite. It is [Boundaries](https://www.destroyallsoftware.com/talks/boundaries) by Gary Bernhardt. There’s a point there though, which I strongly disagree with:
...
Definitely do *not* use conditional compilation to hide slow tests — it’s an obvious solution which makes your life harder ([similar observation](https://peter.bourgon.org/blog/2021/04/02/dont-use-build-tags-for-integration-tests.html) from the Go ecosystem).
...
![](https://matklad.github.io/assets/goto-definition-test.png)
...
Previous example had a complex data input, but a relatively simple data output — a position in the file. Often, the output is messy and has a complicated structure as well (a symptom of [rho problem](https://buttondown.email/hillelwayne/archive/cross-branch-testing/)). Worse, sometimes the output is a part that is changed frequently. This often necessitates updating a lot of tests. Going back to the binary search example, the change from `-> bool` to `-> Result<usize, usize>` was an example of this effect.
...
Often, just `Debug` representation of the type works well for expect tests, but you can do something more fun. See this post from Jane Street for a great example: [Using ASCII waveforms to test hardware designs](https://blog.janestreet.com/using-ascii-waveforms-to-test-hardware-designs/).
...
Tests for caching additionally enable verbose internal logging. In this mode, Cargo prints information about cache hits and misses. These messages are then used [in assertions](https://github.com/rust-lang/cargo/blob/57b75970e022e8519fe82cc38a7aed4862f67089/tests/testsuite/rustc_info_cache.rs#L68-L70).

A close idea is [coverage marks](https://ferrous-systems.com/blog/coverage-marks/). Some times, you want to check that something **does not** happen. Tests for this tend to be fragile — often the thing does not happen, but for the wrong reason. You can add a side channel which explains the reasoning behind particular behavior, and additionally assert this as well.
...
Use random bytes as a seed to generate “syntactically valid” inputs, then see you software crash and burn when the most hideous edge cases are uncovered. If you use Rust, check out [wasm-smith](https://github.com/bytecodealliance/wasm-tools/tree/f632261627a0ea758762e431d8be32740111e33c/crates/wasm-smith) and [arbitrary](https://lib.rs/crates/arbitrary) crates.
...
3. Separate the tricky to test bits into a separate project. This allows you to write slow and not 100% reliable tests for integration parts, while keeping the core test suite fast and dependable.
...
This essay already mentioned a couple of cognitive tricks for better testing: reducing the fixed costs for adding new tests, and plotting/printing test times. The best trick in a similar vein is the [“not rocket science”](https://graydon2.dreamwidth.org/1597.html) rule of software engineering.
...
6. Testing time is something worth optimizing for. Tests are sensitive to IO and IPC. Tests are relatively insensitive to the amount of code under tests.
...
10. Corollary: good tests are helpful for design in the large. They help to crystalize the data model your application is built around.

## Links

1. [https://www.destroyallsoftware.com/talks/boundaries](https://www.destroyallsoftware.com/talks/boundaries)
2. [https://www.tedinski.com/2019/03/19/testing-at-the-boundaries.html](https://www.tedinski.com/2019/03/19/testing-at-the-boundaries.html)
3. [https://programmingisterrible.com/post/139222674273/how-to-write-disposable-code-in-large-systems](https://programmingisterrible.com/post/139222674273/how-to-write-disposable-code-in-large-systems)
4. [https://sans-io.readthedocs.io](https://sans-io.readthedocs.io/)
5. [https://peter.bourgon.org/blog/2021/04/02/dont-use-build-tags-for-integration-tests.html](https://peter.bourgon.org/blog/2021/04/02/dont-use-build-tags-for-integration-tests.html)
6. [https://buttondown.email/hillelwayne/archive/cross-branch-testing/](https://buttondown.email/hillelwayne/archive/cross-branch-testing/)
7. [https://blog.janestreet.com/testing-with-expectations/](https://blog.janestreet.com/testing-with-expectations/)
8. [https://blog.janestreet.com/using-ascii-waveforms-to-test-hardware-designs/](https://blog.janestreet.com/using-ascii-waveforms-to-test-hardware-designs/)
9. [https://ferrous-systems.com/blog/coverage-marks/](https://ferrous-systems.com/blog/coverage-marks/)
10. [https://vorpus.org/blog/notes-on-structured-concurrency-or-go-statement-considered-harmful/](https://vorpus.org/blog/notes-on-structured-concurrency-or-go-statement-considered-harmful/)
11. [https://graydon2.dreamwidth.org/1597.html](https://graydon2.dreamwidth.org/1597.html)
12. [https://bors.tech](https://bors.tech/)
13. [https://fsharpforfunandprofit.com/posts/property-based-testing/](https://fsharpforfunandprofit.com/posts/property-based-testing/)
14. [https://fsharpforfunandprofit.com/posts/property-based-testing-1/](https://fsharpforfunandprofit.com/posts/property-based-testing-1/)
15. [https://fsharpforfunandprofit.com/posts/property-based-testing-2/](https://fsharpforfunandprofit.com/posts/property-based-testing-2/)
16. [https://www.sqlite.org/testing.html](https://www.sqlite.org/testing.html)
...
[https://www.tbray.org/ongoing/When/202x/2021/05/15/Testing-in-2021](https://www.tbray.org/ongoing/When/202x/2021/05/15/Testing-in-2021)
